...
const SITE = 'https://extracurricular.co.kr';

// Copied verbatim from vansuccess.co.kr's scripts/generate-areas.js REGIONS
// dataset (real KR administrative divisions), reused here for the same
// nationwide-targeting approach already used on vansuccess/vendmachine.
const REGIONS = [
...
const GRADES = {
  ...
  high1: { label: '고1', title: '첫 내신 상대평가, 과목 선택 준비', body: '고등학교 상대평가 등급이 처음 매겨지는 시기로, 첫 내신을 관리하며 2학년 선택과목을 위한 진로 방향을 함께 잡습니다.', tags: ['첫 내신', '등급 관리', '과목 선택 준비'] },
  high2: { label: '고2', title: '진로·융합선택과목, 성취평가제 대비', body: '진로선택·융합선택과목이 본격적으로 시작되는 시기로, 성취평가제(절대평가) 과목의 학습법을 상대평가 과목과 구분해 지도합니다.', tags: ['진로선택과목', '융합선택과목', '성취평가제'] },
  high3: { label: '고3', title: '등급 마무리, 학생부 연계 정리', body: '남은 과목의 등급을 마무리하고, 그동안 선택한 과목 이수 현황을 학생부종합전형 등 대입 전형과 연계해 정리하는 시기입니다.', tags: ['등급 마무리', '학생부 연계', '대입 전형'] },
};
...
<meta property="og:site_name" content="1등급만들기">
...
function header() {
  return `<header>
  <div class="wrap nav">
    <a class="logo" href="/"><span class="logo-mark">1</span>1등급만들기</a>
    <a class="nav-cta" href="tel:010-3951-0535">무료 진단 신청 010-3951-0535</a>
  </div>
</header>
...
function footer() {
  return `<footer>
  <div class="wrap">
    <p>© 2026 1등급만들기. All rights reserved. · <a href="/" style="color:#fff;">홈으로</a></p>
  </div>
</footer>`;
}
function faqMini() {
...
function regionPageTemplate(region, grade) {
  ...
  const districtLinks = region.districts.length
    ? region.districts.map(d => `<a href="/areas/${encodeURIComponent(districtSlug(region, d, grade))}.html">${region.name} ${baseName(d)} ${grade.label}과외</a>`).join('\n          ')
    : '';
...
<section>
  <div class="wrap">
    <div class="cat-grid" style="grid-template-columns:1fr;">
      <div class="cat-card"><div class="cat-tag">${esc(grade.label)}</div><h3>${esc(grade.title)}</h3><p>${esc(grade.body)}</p></div>
    </div>
  </div>
</section>
${faqMini()}
${districtLinks ? `<section class="section-alt">
  <div class="wrap">
    <div class="section-head"><h2>${esc(region.name)} 시/군/구별 ${esc(grade.label)}과외</h2></div>
...
<section class="cta-section" id="apply">
  <div class="wrap" style="max-width:520px;">
    <h2>${esc(region.name)} ${esc(grade.label)}과외, 지금 문의하세요</h2>
    <p>상담과 학습 진단은 무료이며, 등록을 강요하지 않습니다.</p>
    <div class="hero-actions" style="justify-content:center; display:flex; gap:12px;">
      <a class="btn-primary" style="background:var(--navy);" href="/#apply">무료 학습 진단 신청</a>
      <a class="btn-ghost" style="border-color:var(--navy); color:var(--navy);" href="tel:010-3951-0535">☎ 전화 상담</a>
    </div>
    <p style="margin-top:20px;">
      <a href="#" onclick="return false;" style="color:var(--slate-2);">${esc(region.name)}의 다른 학년: </a>
      ${otherGrades}
    </p>
    <p style="margin-top:12px;"><a href="/areas/" style="color:var(--crimson); font-weight:700;">전국 지역별 과외 전체 보기 →</a></p>
  </div>
</section>
${footer()}
...
<section>
  <div class="wrap">
    ...
    <p style="margin-top:20px; font-size:13.5px; color:var(--slate);">${esc(district_)}${topicParticle(district_)} <a href="${parentUrl}" style="color:var(--crimson); font-weight:700;">${esc(region.name)}</a> 소속 지역으로, 전국 어디서나 동일한 기준으로 화상 수업이 진행됩니다.${hasSub ? ` ${esc(district_)} 관내 ${esc(subNote)} 등 모든 읍 지역도 동일하게 상담 가능합니다.` : ''}</p>
  </div>
</section>
${faqMini()}
<section class="cta-section" id="apply">
  <div class="wrap" style="max-width:520px;">
    <h2>${esc(district_)} ${esc(grade.label)}과외, 지금 문의하세요</h2>
    <p>상담과 학습 진단은 무료이며, 등록을 강요하지 않습니다.</p>
    <div class="hero-actions" style="justify-content:center; display:flex; gap:12px;">
      <a class="btn-primary" style="background:var(--navy);" href="/#apply">무료 학습 진단 신청</a>
      <a class="btn-ghost" style="border-color:var(--navy); color:var(--navy);" href="tel:010-3951-0535">☎ 전화 상담</a>
    </div>
    <p style="margin-top:20px;"><a href="${parentUrl}" style="color:var(--crimson); font-weight:700;">${esc(region.name)} 전체 보기 →</a> · <a href="/areas/" style="color:var(--crimson); font-weight:700;">전국 지역별 과외 전체 보기 →</a></p>
  </div>
</section>
${footer()}
<script src="/assets/site.js"></script>
...
function hubTemplate() {
  const groups = REGIONS.map(region => {
    const links = GRADE_LIST.map(g => `<a href="/areas/${encodeURIComponent(regionSlug(region, g))}.html">${region.name} ${g.label}과외</a>`).join('\n          ');
    return `      <div style="margin-bottom:28px;">
        <h3 style="margin-bottom:12px;">${esc(region.name)}</h3>
        <div class="cat-grid" style="grid-template-columns:repeat(3,1fr); gap:10px;">
          ${links}
        </div>
      </div>`;
  }).join('\n');
...
console.log(`Generated ${regionCount} region-grade pages + ${districtCount} district-grade pages + 1 hub page.`);
